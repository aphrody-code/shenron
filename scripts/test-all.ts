...
const PASSTHROUGH: string[] = (() => {
  ...
  const vals = ["--seed", "--rerun-each", "--retry", "--max-concurrency", "--test-name-pattern", "-t"];
...
const POLICY: Record<string, Policy> = {
  ...
  "@rpbey/discordy": { mode: "unit", testDir: "tests", perFile: true, preload: ["reflect-metadata"] },
...
async function discoverMembers(): Promise<Member[]> {
  ...
  for (const g of globs) {
    ...
    for await (const rel of glob.scan({ cwd: ROOT, onlyFiles: true, followSymlinks: false })) {
      ...
      members.push({ name, dir, rel: resolve(ROOT, rel, "..").slice(ROOT.length + 1), policy: POLICY[name] ?? { mode: "unit" } });
    }
  }
...
async function runScope(m: Member, files: string[]): Promise<Result> {
  ...
  for (const cmd of m.policy.prep ?? []) {
    ...
    if (code !== 0) {
      return { member: m, status: "PREP-FAIL", tests: files.length, durationMs: (Bun.nanoseconds() - t0) / 1e6, note: `prep failed: ${cmd.join(" ")}` };
    }
  }
  ...
  if (COVERAGE) baseArgs.push("--coverage", "--coverage-reporter=lcov", `--coverage-dir=${join(ROOT, "coverage", safeName(m.name))}`);
  if (JUNIT) {
    baseArgs.push("--reporter=junit", "--reporter-outfile", join(ROOT, "test-results", `${safeName(m.name)}.xml`));
  }
  ...
  return { member: m, status: failed ? "FAIL" : "PASS", tests: files.length, durationMs: (Bun.nanoseconds() - t0) / 1e6 };
}
...
for (const m of selected) {
  ...
  if (mode === "skip") {
    results.push({ member: m, status: "SKIP", tests: files.length, durationMs: 0, note: m.policy.reason ?? "policy: skip" });
    continue;
  }
  if (mode === "live" && !RUN_LIVE) {
    results.push({ member: m, status: "SKIP", tests: files.length, durationMs: 0, note: m.policy.reason ?? "live tier (use --live)" });
    continue;
  }
  if (mode === "vendored" && !RUN_VENDORED) {
    results.push({ member: m, status: "SKIP", tests: files.length, durationMs: 0, note: m.policy.reason ?? "vendored (use --vendored)" });
    continue;
  }
  if (files.length === 0) {
...
const totalTests = results.reduce((n, r) => n + (r.status === "PASS" || r.status === "FAIL" ? r.tests : 0), 0);
...
if (JSON_OUT) {
  console.log(
    JSON.stringify(
      { scopes: results.map((r) => ({ name: r.member.name, rel: r.member.rel, status: r.status, tests: r.tests, ms: Math.round(r.durationMs), note: r.note })) },
      null,
      2,
    ),
  );

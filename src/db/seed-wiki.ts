...
interface RemoteChar {
  ...
  transformations?: Array<{ id: number; name: string; ki: string; image: string }>;
}
...
async function main() {
  ...
  console.log(`✓ ${chars.length} characters + ${totalTransfos} transformations inserted`);
  console.log(`  Images mapped locally: ${imgMapped}, fallback network: ${imgMissing}`);

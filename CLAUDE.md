...
- **`= any(${tableau})` en drizzle/postgres-js** → `malformed array literal: "684"` : le driver sérialise le tableau en scalaire. Développer la liste en paramètres — ``in (${sql.join(ids.map((i) => sql`${i}`), sql`, `)})``.

...
2. **Bun obligatoire** : pas de `node`/`npm`/`pnpm`/`yarn`/`tsx`. Utiliser `bun`, `bunx`, `bun --filter <app> <cmd>`.
   **Une exception opérationnelle** : le **build** du site tourne sous **Node 22** (`scripts/deploy-site.sh`, `scripts/ops/deploy-site.ts` ; `--bun` force l'ancien comportement). Le site reste **servi** par Bun (`next start`) : seule l'étape de build change de runtime. À noter : ce n'est **pas** parce que Node consommerait moins — mesures du 2026-08-14, le build réclame **~10,5 Gio de mémoire anonyme quel que soit le runtime**, et il a été tué sous Node comme sous Bun. Le facteur discriminant est `vm.swappiness` (cf. piège dédié), pas le runtime ; Node est simplement la configuration validée en dernier.

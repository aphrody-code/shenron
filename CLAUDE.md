...
6. **On corrige là où l'on lit — par un lien, pas par une modale.** Chaque rubrique affichée porte un `<Link>` **rendu côté serveur** vers `/wiki/corriger?table=&row=&col=`, qui vise le texte réellement rendu : une section de `db_wiki_sections` s'édite ligne à ligne (`body`), une rubrique issue de l'article s'édite dans l'`article`. L'éditeur n'est instancié qu'une fois, sur cette page — un îlot client par section a tué le build (cf. piège dédié). Corollaire non négociable : les pages qui rendent des panneaux passent `sansArticle` à `WikiEditBar`. Sur les **266 fiches personnage pilotées par `db_wiki_sections`, l'`article` n'est pas rendu du tout** (`buildWikiContentPanels` donne la priorité aux sections DB) — y proposer une correction d'article la ferait accepter sans rien changer à l'écran.
...
- **Ce qu'un module de rendu partagé importe peut tuer le build (mesuré : 3 OOM en une soirée)** : `wiki-panels.tsx` est rendu par ~1 400 pages statiques ; tout ce qu'il tire est multiplié d'autant. **Deux causes distinctes, même symptôme** — code 137 pendant la phase de compilation Turbopack, journal arrêté à « Creating an optimized production build », donc AVANT la moindre page générée :
  1. un import qui traverse les couches — `wiki-revalidate` pour six lignes de correspondance `entityType → table`, ce qui tirait `next/cache` **et tout `wiki-admin`** (Drizzle + specs de toutes les tables). Recopier les six lignes sur place a suffi ;
  2. **un îlot client par rubrique** — un `WikiContribute` (`"use client"`) rendu dans chaque section, soit des milliers d'instances. Remplacé par un `<Link>` rendu côté serveur vers `/wiki/corriger`, qui coûte zéro octet de JS.
  **Le diagnostic ne se fait pas à la RAM** : les six lancements de la soirée annonçaient tous 8,3–9,2 Gio de RAM disponible et 15–16 Gio de swap, et le noyau a tué bun à **6,8–7,3 Gio d'anon-rss** — bien avant les ~10,5 Gio qu'un build sain réclame. Un OOM sous le plafond habituel et avant toute page générée accuse le graphe de modules, pas la machine : chercher ce qui vient d'être ajouté à un module largement partagé (`dmesg -T | grep 'Killed process'` donne l'anon-rss réel, `wc -l` sur le journal de build dit jusqu'où il est allé).

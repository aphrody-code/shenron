...
window.drupalTranslations = {
	strings: {
		"": {
			...
			Refresh: "Refresh",
			Apply: "Apply",
			"Previous reward": "Previous reward",
			"Next reward": "Next reward",
			"Previous news": "Previous article",
			"Next news": "Next article",
...
(function (Drupal, drupalSettings, drupalTranslations, console, Proxy, Reflect) {
	...
	Drupal.formatPlural = function (count, singular, plural, args, options) {
		...
		const translations = Drupal.t(singular + pluralDelimiter + plural, args, options).split(
			pluralDelimiter
		);
		let index = 0;
	...
	Drupal.theme = function (func, ...args) {
		if (typeof Drupal.theme?.[func] === "function") return Drupal.theme[func](...args);
		Drupal.throwError(new TypeError(`Drupal.theme.${func} must be function type.`));
	};

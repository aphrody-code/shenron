...
(function (global, factory) {
			...
			if (element.getAttribute("data-background-image"))
				...
				if (element.getAttribute("data-background-image-set")) {
					var imageSetLinks = element
						...
						.split(backgroundImageDelimiter);
					var firstUrlLink =
						imageSetLinks[0].substr(0, imageSetLinks[0].indexOf(" ")) || imageSetLinks[0];
					firstUrlLink =
						firstUrlLink.indexOf("url(") === -1 ? "url(" + firstUrlLink + ")" : firstUrlLink;
					if (imageSetLinks.length === 1) element.style.backgroundImage = firstUrlLink;
					else
	...
	function lozad() {
		...
		for (var i = 0; i < elements.length; i++) preLoad(elements[i]);
		return {
			observe: function observe() {
				var elements = getElements(selector, root);
				for (var _i = 0; _i < elements.length; _i++) {
					if (isLoaded(elements[_i])) continue;
					if (observer) {

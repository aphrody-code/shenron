		...
		(e.ui.focusable = function (t, i) {
			...
			return "area" === r
				...
				: (/^(input|select|textarea|button|object)$/.test(r)
						? (a = !t.disabled) && (o = e(t).closest("fieldset")[0]) && (a = !o.disabled)
						: (a = ("a" === r && t.href) || i),
					a && e(t).is(":visible") && "visible" === e(t).css("visibility"));
		}),
			...
			? this[0].labels && this[0].labels.length
				...
				: ((i = this.eq(0).parents("label")),
					(e = this.attr("id")) &&
						...
						(s = "label[for='" + CSS.escape(e) + "']"),
						(i = i.add(n.find(s).addBack(s)))),
					this.pushStack(i))
			: this.pushStack([]);
	});
});
			...
			_mouseInit: function () {
				...
				(this.element
					...
					.on("click." + this.widgetName, function (s) {
						if (!0 === e.data(s.target, t.widgetName + ".preventClickEvent"))
							return (
			...
			_mouseDown: function (s) {
				if (!t) {
					...
					return (
						...
						((this._mouseStarted = !1 !== this._mouseStart(s)), !this._mouseStarted)
							? (s.preventDefault(), !0)
							: (!0 === e.data(s.target, this.widgetName + ".preventClickEvent") &&
									e.removeData(s.target, this.widgetName + ".preventClickEvent"),
			...
			_createHelper: function (e) {
				...
				return (
					o.parents("body").length ||
						o.appendTo("parent" === s.appendTo ? this.element[0].parentNode : s.appendTo),
			...
			start: function (e, s, i) {
				...
				((i.sortables = []),
					t(i.options.connectToSortable).each(function () {
						...
						s &&
							!s.options.disabled &&
							(i.sortables.push(s), s.refreshPositions(), s._trigger("activate", e, o));
					}));
			},
			stop: function (e, s, i) {
				var o = t.extend({}, s, { item: i.element });
					...
					((h = (a = i.snapElements[f].left - i.margins.left) + i.snapElements[f].width),
						...
						!t.contains(i.snapElements[f].item.ownerDocument, i.snapElements[f].item)
							...
							: ("inner" !== g.snapMode &&
									...
									l &&
										(s.position.left = i._convertPositionTo("relative", { top: 0, left: h }).left)),
								(d = o || n || r || l),
								"outer" !== g.snapMode &&
									((o = Math.abs(p - _) <= u),
									(n = Math.abs(c - P) <= u),
									...
									r &&
										(s.position.left = i._convertPositionTo("relative", { top: 0, left: a }).left),
									l &&
										(s.position.left = i._convertPositionTo("relative", {
											top: 0,
			...
			start: function (e, s, i) {
				var o,
					...
					r = t.makeArray(t(n.stack)).sort(function (e, s) {
						return (
							(parseInt(t(e).css("zIndex"), 10) || 0) - (parseInt(t(s).css("zIndex"), 10) || 0)
						);
					});
				r.length &&
...
!(function (t) {
	...
	"function" == typeof define && define.amd
		? define(
				["jquery", "./mouse", "../disable-selection", "../plugin", "../version", "../widget"],
				t
			)
		: t(jQuery);
})(function (t) {
		...
		t.widget("ui.resizable", t.ui.mouse, {
			...
			_setupHandles: function () {
				...
				if (
					...
					for (
						...
						((h = "ui-resizable-" + (e = String.prototype.trim.call(s[i]))),
							(n = t("<div>")),
			...
			_mouseCapture: function (e) {
				...
				for (i in this.handles)
					((s = t(this.handles[i])[0]) === e.target || t.contains(s, e.target)) && (h = !0);
			...
			_mouseStart: function (e) {
				...
				return (
					...
					this._helper || (n = this._calculateAdjustedElementDimensions(a)),
			...
			_mouseStop: function (e) {
				...
				return (
					this._helper &&
						...
						(a =
							parseFloat(p.element.css("left")) + (p.position.left - p.originalPosition.left) ||
							null),
						(l =
							parseFloat(p.element.css("top")) + (p.position.top - p.originalPosition.top) || null),
			...
			_getPaddingPlusBorderDimensions: function (t) {
				for (
					...
					((i[e] = parseFloat(s[e]) || 0), (i[e] += parseFloat(h[e]) || 0));
			...
			_calculateAdjustedElementDimensions: function (t) {
				...
				return "content-box" !== t.css("box-sizing") ||
					(!this._hasScroll(h) && !this._hasScroll(h, "left"))
					? { height: parseFloat(t.css("height")), width: parseFloat(t.css("width")) }
					: ((e = parseFloat(h.style.width)),
						(i = parseFloat(h.style.height)),
			...
			resize: function (e) {
				...
				(g[0] !== document && /static/.test(g.css("position")) && (d = l),
					r.left < (o._helper ? l.left : 0) &&
						((o.size.width =
							o.size.width + (o._helper ? o.position.left - l.left : o.position.left - d.left)),
						p && ((o.size.height = o.size.width / o.aspectRatio), (u = !1)),
						(o.position.left = a.helper ? l.left : 0)),
					r.top < (o._helper ? l.top : 0) &&
						((o.size.height =
							o.size.height + (o._helper ? o.position.top - l.top : o.position.top)),
					...
					(i = Math.abs(
						o.sizeDiff.width + (o._helper ? o.offset.left - d.left : o.offset.left - l.left)
					)),
					(s = Math.abs(
						o.sizeDiff.height + (o._helper ? o.offset.top - d.top : o.offset.top - l.top)
					)),
		...
		t.ui.plugin.add("resizable", "grid", {
			resize: function () {
				...
				((s.grid = l),
					...
					m && (c -= p),
					/^(se|s|e)$/.test(a)
						? ((i.size.width = u), (i.size.height = c))
						: /^(ne)$/.test(a)
							? ((i.size.width = u), (i.size.height = c), (i.position.top = o.top - g))
							: /^(sw)$/.test(a)
								? ((i.size.width = u), (i.size.height = c), (i.position.left = o.left - d))
								: ((c - p <= 0 || u - r <= 0) && (e = i._getPaddingPlusBorderDimensions(this)),
									c - p > 0
										? ((i.size.height = c), (i.position.top = o.top - g))
										: ((c = p - e.height),
											(i.size.height = c),
											(i.position.top = o.top + n.height - c)),
									u - r > 0
		...
		i.widget("ui.dialog", {
			...
			close: function (t) {
				...
				this._isOpen &&
					...
					this.opener.filter(":focusable").trigger("focus").length ||
						i(this.document[0].activeElement).trigger("blur"),
			...
			_setOption: function (t, e) {
				...
				"disabled" !== t &&
					...
					"modal" === t && n.attr("aria-modal", e ? "true" : null));
			},
			_size: function () {
				var i,
					t,
	...
	return (
		t.widget("ui.tabs", {
			...
			_findNextTab: function (e, i) {
				...
				for (; -1 !== t.inArray((e > a && (e = 0), e < 0 && (e = a), e), this.options.disabled); )
					e = i ? e + 1 : e - 1;
				return e;
			},
				...
				((this.tablist = this._getList().attr("role", "tablist")),
					...
					this.anchors.each(function (i, a) {
						var s,
							n,
							h,
							r = t(a).uniqueId().attr("id"),
							o = t(a).closest("li"),
							l = o.attr("aria-controls");
						(e._isLocal(a)
							? ((h = (s = a.hash).substring(1)), (n = e.element.find(e._sanitizeSelector(s))))
							: ((s = "#" + (h = o.attr("aria-controls") || t({}).uniqueId()[0].id)),
								(n = e.element.find(s)).length ||
									(n = e._createPanel(h)).insertAfter(e.panels[i - 1] || e.tablist),
								n.attr("aria-live", "polite")),
							n.length && (e.panels = e.panels.add(n)),
							l && o.data("ui-tabs-aria-controls", l),
							o.attr({ "aria-controls": h, "aria-labelledby": r }),
			...
			_eventHandler: function (e) {
				var i = this.options,
					...
					l = { oldTab: a, oldPanel: o, newTab: h ? t() : s, newPanel: r };
				(e.preventDefault(),
					s.hasClass("ui-state-disabled") ||
						...
						((i.active = !h && this.tabs.index(s)),
						(this.active = n ? t() : s),
						this.xhr && this.xhr.abort(),
						o.length || r.length || t.error("jQuery UI Tabs: Mismatching fragment identifier."),
			...
			_toggle: function (e, i) {
				...
				function r() {
					(a._addClass(i.newTab.closest("li"), "ui-tabs-active", "ui-state-active"),
						s.length && a.options.show ? a._show(s, a.options.show, h) : (s.show(), h()));
				}
				((this.running = !0),
			...
			_getIndex: function (e) {
				return (
					"string" == typeof e &&
						(e = this.anchors.index(this.anchors.filter("[href$='" + t.escapeSelector(e) + "']"))),
					e
				);
			},
			_destroy: function () {
			...
			load: function (e, i) {
				...
				this._isLocal(n[0]) ||
					((this.xhr = t.ajax(this._ajaxSettings(n, i, r))),
					this.xhr &&
			...
			_ajaxSettings: function (e, i, a) {
				...
				return {
					...
					beforeSend: function (e, n) {
						return s._trigger("beforeLoad", i, t.extend({ jqXHR: e, ajaxSettings: n }, a));
					},
				};
			},
	...
	return (
		t.widget("ui.resizable", t.ui.mouse, {
			...
			_mouseStart: function (i) {
				...
				return (
					...
					(s = this._num(this.helper.css("top"))),
					n.containment &&
						((e += t(n.containment).scrollLeft() || 0), (s += t(n.containment).scrollTop() || 0)),
					(this.offset = this.helper.offset()),
		...
		t.ui.plugin.add("resizable", "ghost", {
			...
			resize: function () {
				var i = t(this).resizable("instance");
				i.ghost &&
					i.ghost.css({ position: "relative", height: i.size.height, width: i.size.width });
			},
			stop: function () {
				var i = t(this).resizable("instance");
...
!(function (e, t) {
	"object" == typeof exports && "undefined" != typeof module
		...
		: "function" == typeof define && define.amd
			? define(["exports"], t)
			: t(((e = "undefined" != typeof globalThis ? globalThis : e || self).bodyScrollLock = {}));
})(this, function (e) {
	"use strict";
	var t = function () {
	...
	function i() {
		return (
			"__BSL_PREVENT_DEFAULT__" in window ||
				(window.__BSL_PREVENT_DEFAULT__ = function (e) {
	...
	var c = {
		...
		initialClientPos: { clientX: 0, clientY: 0 },
	};
	function r(e) {
		if (t()) return c;
		...
		var n =
			"__BSL_LOCK_STATE__" in window
				? Object.assign(Object.assign({}, c), window.__BSL_LOCK_STATE__)
				: c;
	...
	function d(e) {
		var t = document.documentElement,
			...
			o = (void 0, !0);
		if (((t.style.overflow = "hidden"), (t.style.boxSizing = "border-box"), o)) {
			var i = window.innerWidth - t.clientWidth,
				l = parseInt(window.getComputedStyle(t).paddingRight, 10);
			t.style.paddingRight = "".concat(i + l, "px");
		}
		return function () {
	...
	function s(e) {
		e.documentListenerAdded &&
			(document.removeEventListener("touchmove", i(), o({ passive: !1 })),
	...
	((r.lockState = c),
		(e.clearBodyLocks = function (e) {
			...
			if (!t()) {
				var i = r(e);
				if (
					((i.lockedNum = 0), null === (o = i.unLockCallback) || void 0 === o || o.call(i), n().ios)
				) {
					if (i.lockedElements.length)
						for (var l = i.lockedElements.pop(); l; )
							((l.ontouchmove = null), (l.ontouchstart = null), (l = i.lockedElements.pop()));
					s(i);
				}
			}
		}),
		...
		(e.lock = function (e, c) {
			if (!t()) {
				...
				(s.ios
					? (l(e)
							.filter(function (e) {
								return e && -1 === u.lockedElements.indexOf(e);
							})
							.forEach(function (e) {
								((e.ontouchstart = function (e) {
									var t = e.targetTouches[0],
										n = t.clientX,
										o = t.clientY;
									u.initialClientPos = { clientX: n, clientY: o };
								}),
									(e.ontouchmove = function (t) {
										!(function (e, t, n) {
											if (t) {
												...
												if (
													(f && ((a > 0 && 0 === o) || (a < 0 && o + s + 1 >= r))) ||
													(!f && ((u > 0 && 0 === l) || (u < 0 && l + d + 1 >= c)))
												)
													return i()(e);
											}
											e.stopPropagation();
										})(t, e, u.initialClientPos);
									}),
									u.lockedElements.push(e));
							}),
						(function (e) {
							if (!n().ios) return;
							if (e.documentListenerAdded) return;
							(document.addEventListener("touchmove", i(), o({ passive: !1 })),
								(e.documentListenerAdded = !0));
						})(u),
						(null == c ? void 0 : c.setOverflowForIOS) && (u.unLockCallback = d()))
					: u.lockedNum <= 0 &&
						(u.unLockCallback = s.android
							? (function (e) {
									var t = document.documentElement,
										n = document.body,
									...
									return (
										...
										(n.style.overflow = (null == e ? void 0 : e.overflowType) || "hidden"),
										function () {
		...
		(e.unlock = function (e, o) {
			var i;
			if (!t()) {
				var c = r(o);
				((c.lockedNum -= 1),
					c.lockedNum > 0 ||
						(null === (i = c.unLockCallback) || void 0 === i || i.call(c),
						n().ios &&
							(l(e).forEach(function (e) {
								var t = c.lockedElements.indexOf(e);
								e &&
									-1 !== t &&
									((e.ontouchmove = null), (e.ontouchstart = null), c.lockedElements.splice(t, 1));
							}),
							s(c))));
			}
		}));
	...
	Drupal.behaviors.betterExposedFiltersAutoSubmit = {
		attach: function (context, settings) {
			...
			if (
				...
				drupalSettings.better_exposed_filters.auto_submit_sort_only
			)
			...
			$(selectors, context)
				.addBack(selectors)
				.find("input:text:not(.hasDatepicker), textarea, select, input:radio, input:checkbox")
				.each(function () {
					const $el = $(this);
					const $tagName = $el[0].tagName;
					if ($tagName === "SELECT") setFocus($el);
					else if (["radio", "checkbox"].includes($el.attr("type"))) setFocus($el);
					else setFocus($el);
				});
			...
			$(selectors, context)
				...
				.each(function (i, e) {
					...
					if (e.hasAttribute("data-bef-auto-submit-delay"))
						...
						if (!e.hasAttribute("data-bef-auto-submit-full-form")) {
							const $container = $e.closest("[data-bef-auto-submit-full-form]");
							if (
								$container.length &&
								$container.get(0).hasAttribute("data-bef-auto-submit-delay")
							) {
					...
					$(once("bef-auto-submit", $needsAutoSubmit))
						.on("change", function (e) {
							return e.target.type === "date" ? dateInputChangeDebounce(e) : triggerSubmit(e);
						})
						.on("keyup", Drupal.debounce(triggerSubmit, autoSubmitDelay, false));
				});
			function triggerSubmit(e) {
				const ignoredKeyCodes = [16, 17, 18, 20, 33, 34, 35, 36, 37, 38, 39, 40, 9, 13, 27, 32];
				...
				if (mediaQuery && !matchMedia(mediaQuery).matches) return true;
				if (
					$target.is(
						"[data-bef-auto-submit-exclude], :submit, .select2-search__field, .chosen-search-input"
					)
				)
					return true;
				let textfieldMinimumLength =
					$target.closest("form").data("bef-auto-submit-minimum-length") || 1;
				let inputTextLength = $target.val().length;
				let textfieldHasMinimumLength =
					($target.is(":text:not(.hasDatepicker), textarea") &&
						inputTextLength >= textfieldMinimumLength) ||
				...
				if (
					...
					if (e.type === "change") {
						...
						if (target && textfieldHasMinimumLength) $submit.click();
						else {
							if (!target) $submit.click();
						}
					}
				}
...
(function ($, Drupal, once) {
	...
	Drupal.behaviors.autosubmitExcludeTextfield = {
		attach: function (context, settings) {
			if (!settings.better_exposed_filters?.autosubmit_exclude_textfield) return;
				...
				$form.on("submit", function () {
					$checkboxes.each(function () {
						...
						if (!$checkbox.is(":checked"))
							$("<input>").attr({ type: "hidden", name, value: 0 }).appendTo($form);
					});
				});
			});
		},
...
let nid = null;
let modalRetailerLinks = new Map();
const DATA_CTA_TYPE_PHYSICAL = "physical";
const DATA_CTA_TYPE_DIGITAL = "digital";
const EDITION_UNAVAILABLE_CLASS = "unavailable";
let $retailerModalContent;
(function ($, Drupal, once) {
	function triggerPlatformChange(lastPlatformId) {
		const $lastPlatformInput = getPlatforms()
			.find("input:not(:disabled)")
			.filter((_, p) => $(p).attr("id") === lastPlatformId);
		if ($lastPlatformInput.length > 0) $lastPlatformInput.click();
		else getPlatforms().find("input:not(:disabled)").eq(0).click();
	}
	function callRetailerApi(params) {
		currentCountry = params.country || currentCountry;
		currentPlatform = params.country ? null : params.platform || currentPlatform;
		toggleFormsProcessing();
		const apiUrl = params.country
			? `/rest/${nid}/country/${currentCountry}/available-platforms`
			: `/rest/${nid}/country/${currentCountry}/platform/${currentPlatform}/available-editions`;
		$.getJSON(apiUrl)
			.done((res) => {
				if (params.country) {
					const lastSelected = getLastSelectedPlatformId();
					resetEditions();
					resetPlatforms(res);
					triggerPlatformChange(lastSelected);
				} else toggleEditions(res["retailer_editions"]);
			})
			.fail((err) => {
				console.error("Error while fetching retailers data", err);
			})
			.always(() => {
				toggleFormsProcessing();
			});
	}
	function setUserCountryFromApi() {
		toggleFormsProcessing();
		const apiUrl = `/rest/user-country`;
		$.getJSON(apiUrl)
			.done((res) => {
				const possibleValues = getCountrySelect()
					.find("option")
					.filter(function (index, element) {
						return $(element).val() === res;
					});
				getCountrySelect().val();
				if (possibleValues.length > 0) getCountrySelect().val(res);
				callRetailerApi({ country: getCountrySelect().val() });
			})
			.fail((err) => {
				console.error("Error while fetching retailers data", err);
			})
	...
	function mapEditionFromApi(apiEdition) {
		const physical = apiEdition
			.filter((edition) => edition.digital === "0")
			.map((edition) => cleanObject(edition, ["label", "value", "highlight", "logo"]));
		const digital = apiEdition
			.filter((edition) => edition.digital === "1")
			.map((edition) => cleanObject(edition, ["label", "value", "highlight", "logo"]));
		return { available: true, physical, digital };
	}
	function retailerModalListener() {
		const $editionButtons = getEditions().find(".js-retailer-edition__cta");
		prepareModal($retailerModalContent, $editionButtons, {
			withCloser: false,
			modalClass: "retailer-list",
		});
		$editionButtons.click(function () {
			$retailerModalContent.css("display", "block");
			$retailerModalContent.data(
				"edition",
				$(this).parents(".js-retailer-edition").find(".js-retailer-edition__title").data("edition")
			);
			addRetailerLinksToModal(modalRetailerLinks.get($(this).data("id")));
		});
	}
	function imageModalListener() {
		const $editionPictures = getEditions().find(".js-retailer-edition__img-container");
		$editionPictures.each((_, c) => {
			const $pictureContainer = $(c);
			const $togglers = $pictureContainer.find(
				".js-retailer-edition__img-cta, .js-retailer-edition__img"
			);
			const $picture = $pictureContainer.find(".js-retailer-edition__img");
			prepareModal($picture, $togglers, {
				copyElement: true,
				modalClass: "modal--container--fullscreen",
			});
		});
	}
	function onTitlePlatformClick() {
		const $titlePlatforms = $(".view-platform-title .js-platform");
		$titlePlatforms.click(function onPlatformClick() {
			const platformName = $(this).find(".js-platform__name").text().trim();
			if (currentCountry !== null) {
				const $relatedPlatform = getPlatforms()
					.find(".js-block-retailers__platform-name")
					.filter(function () {
						return $(this).text().trim().toLowerCase() === platformName.toLowerCase();
					})
					.first();
				if ($relatedPlatform.length) $relatedPlatform.click();
			}
		});
	}
	function toggleFormsProcessing() {
		const processingClass = "js-processing";
		getPlatforms().toggleClass(processingClass);
		getCountrySelect().toggleClass(processingClass);
		getEditionSelect().toggleClass(processingClass);
		getEditions().toggleClass(processingClass);
	}
	function resetPlatforms(platformList) {
		currentPlatform = null;
		showSelectedPlatform(null);
		getPlatforms().each((_, p) => {
			const $platform = $(p);
			const $platformInput = $platform.find("input");
			const found = platformList.find((x) => x.id === $platformInput.attr("id"));
			const isInputShown = found !== undefined;
			$platform.css("display", isInputShown ? "" : "none");
			$platformInput.prop("checked", false);
			$platformInput.prop("disabled", !isInputShown);
		});
	}
	function resetEditions() {
		getEditions().each((_, e) => {
			const $edition = $(e);
			$edition.removeClass(EDITION_UNAVAILABLE_CLASS);
			updateEditionCtas(false, $edition, null);
		});
	}
	function toggleEditions(editionList) {
		const cleanedEditionList = cleanObject(editionList, [], mapEditionFromApi);
		const $editions = getEditions();
		const $editionsAllCtas = $editions.find(".js-retailer-edition__cta");
		$editionsAllCtas.each(function (index) {
			$(this).data("id", index);
		});
		$editions.each((_, e) => {
			const $edition = $(e);
			const editionData = cleanedEditionList[$edition.attr("data-edition")];
			const isEditionAvailable = editionData !== undefined ? editionData.available : false;
			$edition.toggleClass(EDITION_UNAVAILABLE_CLASS, !isEditionAvailable);
			updateEditionCtas(isEditionAvailable, $edition, editionData);
		});
	}
	function updateEditionCta($cta, availableEdition, data, dataCtaType) {
		const disablingClass = "disabled";
		const hidingClass = "hidden";
		if (availableEdition) {
			let dataCta;
			switch (dataCtaType) {
				case DATA_CTA_TYPE_PHYSICAL:
					dataCta = data.physical;
					break;
				case DATA_CTA_TYPE_DIGITAL:
					dataCta = data.digital;
					break;
			}
			const ctaIsValid = Object.keys(dataCta).length > 0;
			$cta.toggleClass(disablingClass, !ctaIsValid);
			$cta.toggleClass(hidingClass, !ctaIsValid);
			if (ctaIsValid) modalRetailerLinks.set($cta.data("id"), dataCta);
			else modalRetailerLinks.set($cta.data("id"), "");
			$cta.prop("disabled", !ctaIsValid);
		} else {
			$cta.addClass(disablingClass);
			$cta.removeClass(hidingClass);
			$cta.prop("disabled", true);
			modalRetailerLinks.set($cta.data("id"), null);
		}
	}
	function updateEditionCtas(availableEdition, $edition, data) {
		const $editionPhysicalCta = $edition.find(".js-retailer-edition__cta--physical");
		const $editionDigitalCta = $edition.find(".js-retailer-edition__cta--digital");
		$editionPhysicalCta.each((_, cta) => {
			updateEditionCta($(cta), availableEdition, data, DATA_CTA_TYPE_PHYSICAL);
		});
		$editionDigitalCta.each((_, cta) => {
			updateEditionCta($(cta), availableEdition, data, DATA_CTA_TYPE_DIGITAL);
		});
	}
	function showSelectedPlatform(platform) {
		const checkedClass = "checked";
		getPlatforms().removeClass(checkedClass);
		if (platform) $(platform).addClass(checkedClass);
	}
	function addRetailerLinksToModal(dataRetailerLinks) {
		const $retailerList = $(".js-retailer-modal__retailer-list");
		$retailerList.empty();
		const retailerLinkBaseClasses = "retailer-modal__retailer js-retailer-modal__retailer";
		dataRetailerLinks.forEach((retailer) => {
			const retailerLinkClass =
				retailer.highlight === "1"
					? `${retailerLinkBaseClasses} retailer-modal__retailer--highlight`
					: retailerLinkBaseClasses;
			const retailerLink = `<a href="${retailer.value}"
        target="_blank"
        class="${retailerLinkClass}"
        >
        <span class="retailer-modal__retailer-logo">${retailer.logo}</span>
        <span class="retailer-modal__retailer-name js-retailer-modal__retailer-name">${retailer.label}</span>
        <span class="retailer-modal__retailer-icon-external" aria-hidden="true"></span>
      </a>`;
			$retailerList.append(retailerLink);
		});
	}
	function changeEdition(editionId) {
		const $editions = getEditions();
		$editions.removeClass("js-retailer-edition--visible");
		$editions.filter(`[data-edition="${editionId}"]`).addClass("js-retailer-edition--visible");
	}
	function getPlatforms() {
		return $container.find(".js-block-retailers__platform");
	}
	function getCountrySelect() {
		return $container.find(".js-block-retailers__country-select");
	}
	function getEditionSelect() {
		return $container.find(".js-block-retailers__edition-select");
	}
	function getEditions() {
		return $container.find(".js-retailer-edition");
	}
	function getLastSelectedPlatformId() {
		const lastSelected = getPlatforms().find("input:checked")[0];
		return lastSelected ? $(lastSelected).attr("id") : defaultPlatform;
	}
	function cleanObject(initial, keys = [], modifier = (v) => v) {
		const allowedKeys =
			keys.length > 0
				? Object.keys(initial).filter((key) => keys.includes(key))
				: Object.keys(initial);
		return allowedKeys.reduce((cleanObject, key) => {
			cleanObject[key] = modifier(initial[key]);
			return cleanObject;
		}, {});
	}
	Drupal.behaviors.initRetailers = {
		attach: (context) => {
			$(once("initRetailersBehavior", ".js-block-retailers", context)).each(function () {
				$container = $(this);
				$retailerModalContent = $("#retailer-modal");
				let $comparisonChartButton = $("#js-block-retailers__edition-comparison-chart-button");
				if ($comparisonChartButton.length > 0) {
					let $comparisonChartImage = $("#js-block-retailers__edition-comparison-chart-image");
					prepareModal($comparisonChartImage, $comparisonChartButton, {
						modalClass: "modal--container--fullscreen",
					});
				}
				nid = $container.attr("data-nid");
				defaultPlatform = getPlatforms()
					.filter((_, p) => $(p).hasClass("default"))
					.find("input")
					.attr("id");
				setUserCountryFromApi();
				function setDescriptionToggle() {
					const $togglers = $container.find(".js-retailer-edition__description-toggler");
				...
				onTitlePlatformClick();
				retailerModalListener();
				imageModalListener();
				getPlatforms().change(function (e) {
					showSelectedPlatform(this);
					callRetailerApi({ platform: e.target.id });
				});
				getCountrySelect().change((e) => {
					callRetailerApi({ country: e.target.value });
				});
				getEditionSelect().change((e) => {
...
!(function (e, i) {
	e.behaviors.initTitleTeaserVideo = {
		attach: (e) => {
			i("initTitleTeaserVideoBehavior", "#js-teaser-video__link", e).forEach(function (e) {
				var i = document.getElementById("js-teaser-video__video");
				i &&
					lozad(i, {
						loaded: function (e) {
							var i = document.getElementById("js-teaser-video__image");
							(i && i.classList.add("js-teaser-video__image--hidden"), e.play());
						},
					}).observe();
				var s,
					t,
					...
					d,
					r,
					c = document.getElementById("js-teaser-video__link");
				function u(e) {
					((t = document.getElementById("js-teaser-video__iframe")),
						c.classList.contains("js-teaser-video__link--waiting-cookies-allowed") &&
							(c.classList.remove("js-teaser-video__link--waiting-cookies-allowed"),
							v(t, e.target)),
						c.addEventListener(
							"click",
							function () {
				...
				function f(e) {
					e.data == YT.PlayerState.ENDED && screenfull.exit();
				}
				function v(e, i) {
					screenfull.isEnabled ? (screenfull.request(e), i.playVideo()) : _();
				}
				function _() {
					c.classList.add("js-teaser-video__link--no-fullscreen");
				}
				c &&
					((s = !1),
					(n = document
						.getElementById("js-teaser-video__iframe-container")
						.querySelector(".js-iframe-youtube")),
					(o = n.dataset.youtubeVideoId),
					...
					screenfull.isEnabled &&
						screenfull.on("change", () => {
							...
							screenfull.element.classList.contains("teaser-video__iframe")
								? (d = !0)
								: d && (r.pauseVideo(), (d = !1));
						}),
					screenfull.isEnabled &&
					...
					screenfull.isEnabled
						? c.addEventListener(
								...
								function () {
									s ||
										((function (e) {
											e && (e.classList.add("js-teaser-video__video--hidden"), e.pause());
										})(i),
										c.classList.add("js-teaser-video__link--before-cookies-allowed"));
								},
								!1
							)
						: c.addEventListener(
					...
					(window.onYouTubeIframeAPIReady = function () {
						...
						((s = !0),
							screenfull.isEnabled &&
								(c.classList.contains("js-teaser-video__link--before-cookies-allowed") &&
									((e = i) && (e.classList.remove("js-teaser-video__video--hidden"), e.play()),
									c.classList.remove("js-teaser-video__link--before-cookies-allowed"),
									c.classList.add("js-teaser-video__link--waiting-cookies-allowed")),
								(r = new YT.Player("js-teaser-video__iframe", {
									height: l,
									width: a,
									videoId: o,
									...
									events: { onReady: u, onStateChange: f },
								}))));
					}));
			});
		},
...
(function ($, Drupal) {
	...
	Drupal.Message.getMessageTypeClasses = function () {
		return { status: "success", error: "danger", warning: "warning", info: "info" };
	};
	...
	Drupal.Message.getMessageTypeRoles = function () {
		return { status: "status", error: "alert", warning: "alert", info: "status" };
	};
	Drupal.theme.message = function (message, options) {
		options = options || {};
		var wrapper = Drupal.theme(
			"messageWrapper",
	...
	Drupal.theme.messageWrapper = function (id, type) {
		...
		if (label) wrapper.appendChild(Drupal.theme("messageLabel", label));
		return wrapper;
	};
...
(function ($, window, Drupal, drupalSettings) {
		...
		if ($glyphicon[0]) {
			...
			if ($.fn.tooltip && drupalSettings.bootstrap.tooltip_enabled) {
				$glyphicon
					...
					.removeAttr("title")
					.tooltip("destroy");
				if (message) $glyphicon.attr("data-toggle", "tooltip").attr("title", message).tooltip();
			}
	...
	Drupal.Ajax.prototype.setProgressIndicatorThrobber = function () {
		...
		else $element.append(this.progress.element);
	};
	var success = Drupal.Ajax.prototype.success;
	Drupal.Ajax.prototype.success = function (response, status) {
		if (this.progress.element) this.progress.element.parent().find(".message").remove();
...
("use strict");
let accessibleModalIndex = 0;
let activeAccessibleModal = null;
let globalAccessibleModalListenersRegistered = false;
...
function isHiddenFromAccessibilityTree(element, boundary = document.documentElement) {
	let node = element;
	while (node && node instanceof HTMLElement && node !== boundary) {
...
function getFocusableElements($root) {
	const $modalContainer = $root.hasClass("modal--container")
		? $root
		: $root.closest(".modal--container");
	const modalIsOpen = $modalContainer.length > 0 && $modalContainer.hasClass("on");
	const modalElement = modalIsOpen ? $modalContainer[0] : null;
	return $root.find(FOCUSABLE_ELEMENTS).filter(function filterVisibleTabbableFocusable() {
		if (!isTabbable(this)) return false;
		const visibilityBoundary = modalElement?.contains(this)
			? modalElement
			: document.documentElement;
		if (isHiddenFromAccessibilityTree(this, visibilityBoundary)) return false;
		if (isElementVisible(this)) return true;
		return modalElement !== null && modalElement.contains(this);
	});
}
function isTabbable(element) {
	if (!(element instanceof HTMLElement)) return false;
	if (element.hasAttribute("disabled") || isHiddenFromAccessibilityTree(element)) return false;
	return element.tabIndex >= 0;
}
function getFocusableElementIndex(focusableElements, activeElement) {
	const directIndex = focusableElements.indexOf(activeElement);
	if (directIndex !== -1) return directIndex;
	for (let index = 0; index < focusableElements.length; index++)
		if (focusableElements[index].contains(activeElement)) return index;
	return -1;
}
function findNextTabbableInModal(activeElement, modalElement, focusableElements) {
	const activeIndex = getFocusableElementIndex(focusableElements, activeElement);
	if (activeIndex !== -1) return focusableElements[activeIndex + 1] ?? null;
	if (!modalElement.contains(activeElement)) return null;
	for (const focusable of focusableElements)
		if (activeElement.compareDocumentPosition(focusable) & Node.DOCUMENT_POSITION_FOLLOWING)
			return focusable;
	return null;
}
function findPreviousTabbableInModal(activeElement, modalElement, focusableElements) {
	const activeIndex = getFocusableElementIndex(focusableElements, activeElement);
	if (activeIndex !== -1) return focusableElements[activeIndex - 1] ?? null;
	if (!modalElement.contains(activeElement)) return null;
	for (let index = focusableElements.length - 1; index >= 0; index--) {
		const focusable = focusableElements[index];
		if (activeElement.compareDocumentPosition(focusable) & Node.DOCUMENT_POSITION_PRECEDING)
			return focusable;
	}
	return null;
}
function trapFocus(event, $modalContainer) {
	if (event.key !== "Tab") return;
	const focusableElements = getFocusableElements($modalContainer).toArray();
	if (!focusableElements.length) return;
	const firstFocusable = focusableElements[0];
	const lastFocusable = focusableElements[focusableElements.length - 1];
	const activeElement = document.activeElement;
	const modalElement = $modalContainer[0];
	if (!modalElement.contains(activeElement)) {
		event.preventDefault();
		event.stopImmediatePropagation();
		focusElementAccessibly(event.shiftKey ? lastFocusable : firstFocusable);
		return;
	}
	const activeIndex = getFocusableElementIndex(focusableElements, activeElement);
	const activeIsProgrammaticFocusTarget = activeIndex === -1;
	if (event.shiftKey) {
		const previousTabbable =
			activeIndex > 0
				? focusableElements[activeIndex - 1]
				: findPreviousTabbableInModal(activeElement, modalElement, focusableElements);
		if (!previousTabbable) {
			event.preventDefault();
			event.stopImmediatePropagation();
			focusElementAccessibly(lastFocusable);
			return;
		}
		if (activeIsProgrammaticFocusTarget) {
			event.preventDefault();
			event.stopImmediatePropagation();
			focusElementAccessibly(previousTabbable);
		}
		return;
	}
	const nextTabbable =
		activeIndex !== -1 && activeIndex < focusableElements.length - 1
			? focusableElements[activeIndex + 1]
			: findNextTabbableInModal(activeElement, modalElement, focusableElements);
	if (!nextTabbable) {
		event.preventDefault();
		event.stopImmediatePropagation();
		focusElementAccessibly(firstFocusable);
		return;
	}
	if (activeIsProgrammaticFocusTarget) {
		event.preventDefault();
		event.stopImmediatePropagation();
		focusElementAccessibly(nextTabbable);
	}
}
function containFocusInModal(event, $modalContainer) {
	if ($modalContainer[0].contains(event.target)) return;
	const focusableElements = getFocusableElements($modalContainer).toArray();
	if (focusableElements.length) focusElementAccessibly(focusableElements[0]);
}
function onGlobalAccessibleModalKeyDown(event) {
	if (!activeAccessibleModal) return;
	const $modalContainer = jQuery(activeAccessibleModal.container);
	if (event.key === "Escape" || event.key === "Esc") {
		event.preventDefault();
		event.stopPropagation();
		activeAccessibleModal.close();
		return;
	}
	if (event.key === "Tab") trapFocus(event, $modalContainer);
}
function onGlobalAccessibleModalFocusIn(event) {
	if (!activeAccessibleModal) return;
	containFocusInModal(event, jQuery(activeAccessibleModal.container));
}
function registerGlobalAccessibleModalListeners() {
	if (globalAccessibleModalListenersRegistered) return;
	globalAccessibleModalListenersRegistered = true;
	document.addEventListener("keydown", onGlobalAccessibleModalKeyDown, true);
	document.addEventListener("focusin", onGlobalAccessibleModalFocusIn, true);
}
function setActiveAccessibleModal($modalContainer, closeModal) {
	activeAccessibleModal = { container: $modalContainer[0], close: closeModal };
}
function clearActiveAccessibleModal(closeModal) {
	if (activeAccessibleModal?.close === closeModal) activeAccessibleModal = null;
}
function findAccessibleModalLabelElement($content) {
	const excluded = '.modal--closer, .sr-only, [aria-hidden="true"]';
	const hasText = (_, element) => jQuery(element).text().trim().length > 0;
	const selectors = [
		"[data-modal-title]",
		'[class*="modal-title"]',
		'[class*="__title"]',
...
function setAccessibleModalAccessibleName($content, labelId, fallbackLabel) {
	const $label = findAccessibleModalLabelElement($content);
	if ($label.is("[data-modal-title]")) {
		const dataTitle = $label.attr("data-modal-title")?.trim();
		if (dataTitle) {
			$content.attr("aria-label", dataTitle);
			$content.removeAttr("aria-labelledby");
			return;
		}
	}
	if ($label.length) {
		if (!$label.attr("id")) $label.attr("id", labelId);
		$content.attr("aria-labelledby", $label.attr("id"));
		$content.removeAttr("aria-label");
		return;
	}
	if (fallbackLabel) {
		$content.attr("aria-label", fallbackLabel);
		$content.removeAttr("aria-labelledby");
		return;
	}
	$content.removeAttr("aria-labelledby");
...
function getAccessibleModalOpener(openers, clickedOpener) {
	const focusableTags = ["a", "button", "li"];
	const openerIsFocusable = (opener) =>
		focusableTags.includes(jQuery(opener).prop("tagName").toLowerCase());
	return openerIsFocusable(clickedOpener)
		? jQuery(clickedOpener)
		: openers.filter((_, opener) => openerIsFocusable(opener));
}
function focusElementAccessibly(element) {
...
function moveFocusIntoModal(
	...
	const applyFocus = () => {
		...
		focusApplied = true;
		$content.off("transitionend.accessible-modal-initial-focus");
		if (triggeredOpener instanceof HTMLElement) triggeredOpener.blur();
		setInitialFocus($modalContainer, $modalCloser, $content, initialFocus);
	};
	$content.one("transitionend.accessible-modal-initial-focus", applyFocus);
	window.setTimeout(applyFocus, 500);
}
function setInitialFocus($modalContainer, $modalCloser, $content, initialFocus) {
	if (initialFocus === "label") {
		const labelId = $content.attr("aria-labelledby");
		let $label = labelId ? jQuery(`#${labelId}`) : jQuery();
		if (!$label.length) $label = findAccessibleModalLabelElement($content);
		if ($label.length) {
			$label.addClass("js-modal--title");
			if ($label.attr("tabindex") === undefined) $label.attr("tabindex", "-1");
			focusElementAccessibly($label[0]);
			return;
		}
	}
	const $focusable = getFocusableElements($modalContainer);
	if ($modalCloser.length) focusElementAccessibly($modalCloser.first()[0]);
	else if ($focusable.length) focusElementAccessibly($focusable.first()[0]);
	else {
		if ($content.attr("tabindex") === undefined) $content.attr("tabindex", "-1");
		focusElementAccessibly($content[0]);
	}
}
function buildAccessibleModal(withCloser, containerClass) {
	const closerLabel = Drupal.t("Close");
	const closerHtml = withCloser
		? `<button type="button" class="modal--closer"><span class="sr-only">${closerLabel}</span></button>`
		: "";
	const modalId = `accessible-modal-${accessibleModalIndex}`;
	const modalTemplate = jQuery(`<div class="modal--container ${containerClass}" id="${modalId}">
      <div class="modal--content" role="dialog" aria-modal="true">
        ${closerHtml}
      </div>
    </div>`);
	jQuery("body").append(modalTemplate);
	accessibleModalIndex++;
...
function openAccessibleModal(
	...
	$modalContainer.toggle(0, () => {
		...
		$modalContainer.css("display", "flex");
		onOpened();
		moveFocusIntoModal($modalContainer, $modalCloser, $content, initialFocus, triggeredOpener);
	});
	if (gtmEvent) Drupal.bnee_gtm.send(gtmEvent);
	document.dispatchEvent(
		new CustomEvent("modal-was-opened", {
			detail: { modalId: $modalContainer.attr("id"), triggeredOpener },
		})
	);
}
function prepareAccessibleModal(element, opener, modalOptions = {}) {
	const openingClass = "on";
	let lastOpener;
	let triggeredOpener;
	...
	const $modalContainer = buildAccessibleModal(options.withCloser, options.modalClass);
	const modalLabelId = `${$modalContainer.attr("id")}-label`;
	const eventContentInModalWasAdded = new CustomEvent("content-in-modal-was-added", {
		detail: { modalId: $modalContainer.attr("id") },
	});
	const $content = $modalContainer.find(".modal--content");
	if (!options.buildWhenOpen) {
		$content.append(options.copyElement ? jQuery(element).clone() : jQuery(element));
		setAccessibleModalAccessibleName($content, modalLabelId, options.accessibleNameFallback);
		document.dispatchEvent(eventContentInModalWasAdded);
	}
	let $modalCloser = $modalContainer.find(".modal--closer");
	const $html = jQuery("html");
	const getFocusRestoreTarget = () => {
		if (options.lastOpenerFocusOnModalClose && lastOpener?.length === 1) return lastOpener[0];
		if (
			previouslyFocusedElement instanceof HTMLElement &&
			document.contains(previouslyFocusedElement) &&
			!$modalContainer[0].contains(previouslyFocusedElement)
		)
			return previouslyFocusedElement;
		return null;
	};
	const closeModal = () => {
		if (!$modalContainer.hasClass(openingClass) || isClosing) return;
		isClosing = true;
		clearActiveAccessibleModal(closeModal);
		...
		if (options.buildWhenOpen)
			$content.children(":not(.modal--closer)").each(function removeModalContent(_, child) {
				child.remove();
			});
		if (options.removeUrlHashOnModalClose)
			history.pushState("", document.title, window.location.pathname + window.location.search);
	};
	$modalContainer.data("accessibleModalClose", closeModal);
	registerGlobalAccessibleModalListeners();
	$modalContainer.on("click.accessible-modal-closer", ".modal--closer", (event) => {
		event.preventDefault();
		closeModal();
	});
	if (options.closeOnOverlayClick)
		$modalContainer.on("click.accessible-modal-overlay", (event) => {
			if (event.target !== $modalContainer[0]) return;
			closeModal();
		});
	const openThisModal = () => {
		previouslyFocusedElement = document.activeElement;
		$modalCloser = $modalContainer.find(".modal--closer");
		if (options.buildWhenOpen) {
			$content.append(options.copyElement ? jQuery(element).clone() : jQuery(element));
			setAccessibleModalAccessibleName($content, modalLabelId, options.accessibleNameFallback);
			document.dispatchEvent(eventContentInModalWasAdded);
		...
		openAccessibleModal(
			...
			() => {
				setActiveAccessibleModal($modalContainer, closeModal);
			}
		);
	};
	if (opener === "delay") setTimeout(openThisModal, options.delay);
	else {
		if (options.openable === true) opener.data("openable", true);
		else opener.data("openable", false);
		jQuery(opener).on("mousedown.accessible-modal-opener", (event) => {
			if (event.button !== 0) return;
			event.preventDefault();
		});
		jQuery(opener).on("click.accessible-modal-opener", function onOpenerClick() {
			triggeredOpener = this;
			lastOpener = getAccessibleModalOpener(opener, this);
			if (jQuery(opener).data("openable") !== true) return;
			document.dispatchEvent(
				new CustomEvent("modal-opener-was-triggered-and-modal-will-be-opened", {
					detail: { modalId: $modalContainer.attr("id"), triggeredOpener },
				})

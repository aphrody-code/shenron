        ...
        <div className="px-6 md:px-12 pb-12 relative">
          <div className="relative -mt-20 md:-mt-24 mb-10 flex flex-col md:flex-row items-center md:items-end gap-6 md:gap-8">
            <div className="relative">
              ...
              <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 bg-dbz-orange text-white px-4 py-1 border-2 border-dbz-orange-dark z-20 shadow-[2px_2px_0px_rgba(0,0,0,0.5)]">
                <span className="font-saiyan text-2xl uppercase whitespace-nowrap">LVL {user.level}</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mb-12">
            <div className="bg-dbz-bg border-2 border-dbz-border p-4 text-center">
              <p className="text-[10px] md:text-xs font-bold text-dbz-blue-light uppercase tracking-widest mb-1">XP Total</p>
              <p className="scouter-text text-xl md:text-3xl text-dbz-orange">{user.xp.toLocaleString()}</p>
            </div>
            <div className="bg-dbz-bg border-2 border-dbz-border p-4 text-center">
              <p className="text-[10px] md:text-xs font-bold text-dbz-blue-light uppercase tracking-widest mb-1">Zénis</p>
              <p className="scouter-text text-xl md:text-3xl text-dbz-yellow">{user.zeni.toLocaleString()}</p>
            </div>
            <div className="bg-dbz-bg border-2 border-dbz-border p-4 text-center">
              <p className="text-[10px] md:text-xs font-bold text-dbz-blue-light uppercase tracking-widest mb-1">Succès</p>
              <p className="scouter-text text-xl md:text-3xl text-white">{user.achievements?.length || 0}</p>
            </div>
            <div className="bg-dbz-bg border-2 border-dbz-border p-4 text-center">
              <p className="text-[10px] md:text-xs font-bold text-dbz-blue-light uppercase tracking-widest mb-1">Objets</p>
              <p className="scouter-text text-xl md:text-3xl text-white">{user.inventory?.length || 0}</p>
            </div>
          </div>
            ...
            <div className="lg:col-span-2 space-y-8 md:space-y-12">
              ...
              <section>
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  )) || <p className="text-sm text-gray-500 font-bold uppercase">Aucun succès débloqué.</p>}
                </div>
              </section>
            </div>
          </div>

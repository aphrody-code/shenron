import { db } from "@/lib/db";
import Link from "next/link";

export default async function WikiLayout({
	children,
}: {
	children: React.ReactNode;
}) {
	const categories = await db.query.wikiCategories.findMany({
		where: (c, { isNull }) => isNull(c.parentId),
		with: {
			children: { with: { pages: true } },
			pages: true,
		},
		orderBy: (c, { asc }) => asc(c.order),
	});

	return (
		<div className="container mx-auto px-4 py-12 flex gap-8">
			<aside className="w-64 flex-shrink-0">
				<nav className="sticky top-12 space-y-6">
					<div className="space-y-4">
						<Link
							href="/wiki"
							className="text-xl font-bold hover:text-fuchsia-400 transition-colors block"
						>
							Wiki DBFR
						</Link>
						<Link
							href="/wiki/dragon-ball"
							className="text-xl font-bold text-dbz-yellow hover:text-dbz-orange transition-colors block"
						>
							Encyclopédie DBZ
						</Link>
					</div>
					{categories.map((cat: any) => (
						<div key={cat.id}>
							<h3 className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-2">
								{cat.name}
							</h3>
							<ul className="space-y-1">
								{cat.pages.map((page: any) => (
									<li key={page.id}>
										<Link
											href={`/wiki/${cat.slug}/${page.slug}`}
											className="text-sm text-gray-300 hover:text-white transition-colors block py-1"
										>
											{page.title}
										</Link>
									</li>
								))}
								{cat.children.map((subCat: any) => (
									<li key={subCat.id} className="pl-3">
										<span className="text-xs text-gray-500 block mb-1">
											{subCat.name}
										</span>
										<ul className="space-y-1 border-l border-dbz-border pl-3">
											{subCat.pages.map((page: any) => (
												<li key={page.id}>
													<Link
														href={`/wiki/${cat.slug}/${subCat.slug}/${page.slug}`}
														className="text-sm text-gray-400 hover:text-white transition-colors block py-1"
													>
														{page.title}
													</Link>
												</li>
											))}
										</ul>
									</li>
								))}
							</ul>
						</div>
					))}
				</nav>
			</aside>
			<main className="flex-1 max-w-3xl">{children}</main>
		</div>
	);
}
export const dynamic = "force-dynamic";

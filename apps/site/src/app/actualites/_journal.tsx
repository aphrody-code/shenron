/**
 * Corps partagé du Journal — rendu par `/actualites` (page 1) ET par
 * `/actualites/page/[n]` (pages suivantes).
 *
 * Pourquoi deux routes plutôt qu'un `?page=` : lire `searchParams` dans une page
 * la rend **dynamique** sous Next 16 (`cache-control: private, no-store`), même
 * avec `export const revalidate`. L'index du journal — la page la plus crawlée
 * du site après la home — était donc recalculé à chaque visite et jamais mise en
 * cache CDN. Segmenter la pagination en route statique rend les deux prérendables.
 */
...
/** URL canonique d'une page du journal (page 1 = la racine, jamais `/page/1`). */
export const journalHref = (p: number) => (p <= 1 ? "/actualites" : `/actualites/page/${p}`);

/** Nombre total de pages publiées — sert aussi à `generateStaticParams`. */
export async function journalPageCount(): Promise<number> {
	const total = await db.$count(postsTable, publicPostFilter());
...
export async function JournalIndex({ page }: { page: number }) {
	...
	const [rows, totalCount, allTags] = await Promise.all([
		...
		// Thèmes réellement utilisés par les articles en ligne (pas de liste figée
		// qui pointerait vers des pages vides).
		db.select({ tags: postsTable.tags }).from(postsTable).where(visible).limit(300),
	]);

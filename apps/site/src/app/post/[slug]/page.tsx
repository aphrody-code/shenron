		...
		<div className="container mx-auto px-4 py-12 max-w-4xl">
			...
			{post.cover && (
				<img
					src={post.cover}
					alt={post.title}
					className="w-full aspect-video object-cover rounded-3xl mb-12 shadow-2xl"
				/>
			)}
			...
			<section className="border-t border-[#1a0d2e] pt-12">
				...
				<div className="space-y-6 mb-12">
					{post.comments.map((comment: any) => (
						<div
							key={comment.id}
							className="p-4 rounded-xl bg-[#1a0d2e]/30 border border-[#1a0d2e]"
						>
							<div className="flex items-center gap-3 mb-2">
								...
								<span className="text-xs text-gray-500">
									{format(comment.createdAt, "d MMM", { locale: fr })}
								</span>
							</div>
				...
				{session ? (
					...
					<div className="p-8 text-center rounded-2xl border border-dashed border-[#1a0d2e]">
						<p className="text-gray-400">
							Connectez-vous avec Discord pour commenter.
						</p>
					</div>
				)}
			</section>

            ...
            <article key={post.id} className="dbz-panel p-6 md:p-8 flex flex-col md:flex-row gap-6 items-start group">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 text-xs md:text-sm font-bold uppercase tracking-wider text-dbz-blue-light">
                  {post.author.avatar && (
                    <img src={post.author.avatar} alt={post.author.username} className="w-8 h-8 rounded-none border-2 border-dbz-blue-light" />
                  )}
                  <span className="text-dbz-orange">{post.author.username}</span>
                  <span>•</span>
                  <time>{format(post.createdAt, "d MMMM yyyy", { locale: fr })}</time>
                </div>
              </div>
              <div className="w-full md:w-auto">
                <Link href={`/post/${post.slug}`} className="dbz-button inline-block w-full text-center">
                  LIRE
                </Link>
              </div>
            </article>

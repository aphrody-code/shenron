				...
				{ragData.results.map((r, i) => (
					...
					>
						...
						<div className="flex justify-between items-start mb-3 gap-2">
							...
							<span className="text-[9px] font-mono text-emerald-400 bg-emerald-950/20 border border-emerald-500/30 px-2 py-0.5 rounded font-bold uppercase shrink-0">
								{r.kind}
							</span>
						</div>
						...
						<div className="mt-4 pt-3 border-t border-white/5 flex flex-wrap justify-between items-center text-[10px] font-mono text-emerald-500/40 gap-2">
							<span>
								SOURCE: {r.url.replace("https://", "").replace("http://", "").split("/")[0]}
							</span>
							<a
								href={r.url}
								target="_blank"

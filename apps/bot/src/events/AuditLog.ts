...
export class AuditLogEventHandler {
  ...
  async onBanAdd([ban]: ArgsOf<"guildBanAdd">) {
    ...
    if (exec?.id === ban.client.user?.id) return;
  ...
  async onMemberUpdate([oldM, newM]: ArgsOf<"guildMemberUpdate">) {
    ...
    if (added.length > 0) fields.push({ name: "Rôle(s) ajouté(s)", value: added.map((r) => `<@&${r}>`).join(" "), inline: false });
    if (removed.length > 0) fields.push({ name: "Rôle(s) retiré(s)", value: removed.map((r) => `<@&${r}>`).join(" "), inline: false });
  ...
  async onChannelDelete([channel]: ArgsOf<"channelDelete">) {
    ...
    const embed = this.logs
      ...
      .addFields(
        { name: "Salon", value: `#${"name" in channel ? channel.name : "?"} (${channel.id})`, inline: true },
        { name: "Type", value: ChannelType[channel.type] ?? String(channel.type), inline: true },
        { name: "Par", value: exec ? `<@${exec.id}>` : "Inconnu", inline: true },
      );
    await this.logs.send(channel.client, "audit", embed);
  ...
  async onChannelUpdate([oldC, newC]: ArgsOf<"channelUpdate">) {
    ...
    if ("topic" in oldC && "topic" in newC && oldC.topic !== newC.topic) {
      fields.push({ name: "Topic", value: `${oldC.topic ?? "*(vide)*"} → ${newC.topic ?? "*(vide)*"}`.slice(0, 1024) });
    }
    ...
    const embed = this.logs
      ...
      .addFields({ name: "Salon", value: `${newC} (${newC.id})`, inline: true }, { name: "Par", value: exec ? `<@${exec.id}>` : "Inconnu", inline: true }, ...fields);
  ...
  async onRoleUpdate([oldR, newR]: ArgsOf<"roleUpdate">) {
    ...
    if (oldR.color !== newR.color) {
      fields.push({ name: "Couleur", value: `\`#${oldR.color.toString(16).padStart(6, "0")}\` → \`#${newR.color.toString(16).padStart(6, "0")}\`` });
    }
    if (oldR.hoist !== newR.hoist) fields.push({ name: "Hoist", value: `${oldR.hoist} → ${newR.hoist}` });
    if (oldR.mentionable !== newR.mentionable) fields.push({ name: "Mentionnable", value: `${oldR.mentionable} → ${newR.mentionable}` });
    if (oldR.permissions.bitfield !== newR.permissions.bitfield) {
      ...
      if (added.length > 0) fields.push({ name: "Permissions ajoutées", value: added.join(", ").slice(0, 1024) });
      if (removed.length > 0) fields.push({ name: "Permissions retirées", value: removed.join(", ").slice(0, 1024) });
    }
    if (fields.length === 0) return;
    const exec = await this.fetchExecutor(newR.guild, AuditLogEvent.RoleUpdate, newR.id);
    const embed = this.logs
      .makeEmbed("✏️ Rôle modifié", 0xfbbf24)
      .addFields({ name: "Rôle", value: `${newR} (${newR.id})`, inline: true }, { name: "Par", value: exec ? `<@${exec.id}>` : "Inconnu", inline: true }, ...fields);
  ...
  private async fetchExecutor(
    ...
    try {
      ...
      const entry = targetId
        ? logs.entries.find((e) => e.targetId === targetId && Date.now() - e.createdTimestamp < 10_000)

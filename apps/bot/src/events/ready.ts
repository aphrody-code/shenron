...
export class ReadyEvent {
  ...
  async ready([client]: [Client]) {
    ...
    this.autoEval.runEvaluationPipeline().catch((err) => {
      logger.error(err as Error, "[AUTO-EVAL] Échec lors du démarrage de l'auto-évaluation au boot");
    });
    ...
    try {
      this.cron.register({
        name: "redis-indexer",
        description: "Indexation périodique de tous les messages, utilisateurs et salons dans Redis",
    ...
    try {
      ...
      logger.error(err as Error, "[AUTO-EVAL] Impossible d'enregistrer la tâche cron d'auto-évaluation");
    }
  }
}

...
export class AutoEvalService {
	...
	async runEvaluationPipeline(): Promise<void> {
		...
		try {
			...
			logger.error(
				err as Error,
				"[AUTO-EVAL] Échec lors de l'exécution du pipeline d'auto-évaluation"
			);
		}
	}
	...
	async checkMetricsAndAlert(): Promise<void> {
		...
		try {
			...
			if (!ragReportRaw || !ownReportRaw) {
				logger.info(
					"[AUTO-EVAL] Rapports d'évaluation insuffisants dans Redis pour envoyer une alerte."
				);
				return;
			}
			...
			if (isRagDegraded || isReliabilityDegraded || isGroundingDegraded) {
				...
				if (channel && channel.isTextBased()) {
					const embed = {
						...
						description:
							"Le Grand Prêtre a observé une baisse de pertinence ou de qualité dans les réponses de nos divinités.",

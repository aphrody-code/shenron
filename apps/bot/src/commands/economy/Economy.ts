...
export class EconomyCommands {
	...
	async shop(interaction: CommandInteraction) {
		...
		if (items.length === 0) {
			await interaction.reply({
				content: "Shop vide pour le moment.",
					...
					.setDescription(
						list.length
							? list
									.map(
										(i) =>
											`**\`${i.key}\`** · ${i.name} — ${formatXP(i.price)} z\n${i.description ?? ""}`
									)
									.join("\n\n")
							: "— Rien en vente —"
					)
	...
	async buy(
		...
		await interaction.reply({
			content: `✅ Objet acheté (${res.price} z). Utilise \`/inventaire equip\` pour l'équiper.`,
		});
	}
	...
	async eprofil(interaction: CommandInteraction) {
		...
		const fmt = (arr: typeof inv) =>
			arr.length ? arr.map((i) => `\`${i.itemKey}\``).join(", ") : "—";
		...
		const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
			new ButtonBuilder()
				.setCustomId("eprofil:equip:card")
				.setLabel("Carte")
				.setEmoji("🖼️")
				.setStyle(ButtonStyle.Secondary),
			new ButtonBuilder()
				.setCustomId("eprofil:equip:badge")
				.setLabel("Badge")
				.setEmoji("🎖️")
				.setStyle(ButtonStyle.Secondary),
			new ButtonBuilder()
				.setCustomId("eprofil:equip:color")
				.setLabel("Couleur")
				.setEmoji("🎨")
				.setStyle(ButtonStyle.Secondary),
			new ButtonBuilder()
				.setCustomId("eprofil:equip:title")
				.setLabel("Titre")
				.setEmoji("📜")
				.setStyle(ButtonStyle.Secondary)
		);
	...
	async equipBtn(interaction: ButtonInteraction) {
		...
		const modal = new ModalBuilder()
			.setCustomId(`eprofil:submit:${type}`)
			.setTitle(`Équiper un(e) ${type}`);
	...
	async fusion(
		...
		if (target.id === interaction.user.id) {
			await interaction.reply({
				content: "Impossible de fusionner avec soi-même.",
				flags: MessageFlags.Ephemeral,
			});
			return;
		}
		if (target.bot) {
			await interaction.reply({
				content: "Les bots ne fusionnent pas.",
				flags: MessageFlags.Ephemeral,
			});
			return;
		}
		...
		if (existing) {
			await interaction.reply({
				content: "Tu es déjà fusionné(e). Fais `/defusion` d'abord.",
				flags: MessageFlags.Ephemeral,
			});
			return;
		}
		...
		const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
			new ButtonBuilder()
				.setCustomId(`fusion:accept:${interaction.user.id}:${target.id}`)
				.setLabel("Accepter")
				.setStyle(ButtonStyle.Success),
			new ButtonBuilder()
				.setCustomId(`fusion:refuse:${interaction.user.id}:${target.id}`)
				.setLabel("Refuser")
	...
	async fusionResponse(interaction: ButtonInteraction) {
		...
		if (interaction.user.id !== targetId) {
			await interaction.reply({
				content: "Seule la cible peut répondre.",
				flags: MessageFlags.Ephemeral,
			});
			return;
		}
		if (action === "refuse") {
			await interaction.update({
				content: `❌ Fusion refusée par <@${targetId}>.`,
				embeds: [],
				components: [],
			});
			return;
	...
	@Slash({
		name: "zeni",
		description: "Admin: donner/retirer des zéni",
	...
	async zeniAdmin(
		...
		@SlashOption({
			name: "role",
			description: "Rôle (tous les membres avec ce rôle)",
		...
		@SlashOption({
			name: "all",
			description: "Appliquer à tous les membres inscrits",
	...
	@Slash({
		name: "custom",
		description: "Admin: donner/retirer un objet custom",
		defaultMemberPermissions: PermissionFlagsBits.Administrator,
	})
	@Guard(AdminOnly)
	async customAdmin(
		...
		@SlashOption({
			name: "all",
			description: "Appliquer à tous les inscrits",
			type: ApplicationCommandOptionType.Boolean,
			required: false,
		})
		all: boolean | undefined,
		...
		await interaction.editReply({
			content: `✅ ${type}:${key} ${action === "give" ? "donné" : "retiré"} à ${targets.length} membre(s).`,
		});
	}
}

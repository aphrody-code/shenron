...
async function runTechniqueSeed() {
	...
	for (const source of sources) {
		...
		for (const file of files) {
			...
			const sections = data.parse?.sections as { line: string; index: string; level: number; anchor: string }[] | undefined;
			...
			for (const item of items) {
				...
				const rawDesc = item.desc.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
	...
	logger.info(`✓ ${techCount} techniques indexed and ${linkCount} character-technique links created.`);
}

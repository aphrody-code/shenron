...
const SEEDS: PageSeed[] = [
	...
	{
		id: "fandom-dragonballs-fr",
		name: "Dragon Balls (FR)",
		page: "Dragon_Ball_(objet)",
		lang: "fr",
	},
	{
		id: "fandom-transformations-fr",
	...
	{
		id: "fandom-dragonballs-en",
		name: "Dragon Balls (EN)",
		page: "Dragon_Ball_(object)",
		lang: "en",
	},
	{
		id: "fandom-transformations-en",
		name: "Transformations (EN)",
		page: "Transformation",
		lang: "en",
	},
];
...
function cleanMediaWikiHtml(html: string): string {
	...
	cleaned = cleaned.replace(
		/<table\b[^>]*class="[^"]*(?:portable-infobox|infobox)[^"]*"[^>]*>[\s\S]*?<\/table>/gi,
		""
	);
	cleaned = cleaned.replace(
		/<aside\b[^>]*class="[^"]*(?:portable-infobox|infobox)[^"]*"[^>]*>[\s\S]*?<\/aside>/gi,
		""
	);

	// Enlever les tables de navigation en bas de page
	cleaned = cleaned.replace(
		/<table\b[^>]*class="[^"]*(?:navbox|ambox)[^"]*"[^>]*>[\s\S]*?<\/table>/gi,
		""
	);

	// Remplacer les balises de titre (h1, h2, h3, h4) par des titres Markdown correspondants
	cleaned = cleaned.replace(
		/<h1\b[^>]*>[\s\S]*?<span class="mw-headline"[^>]*>([\s\S]*?)<\/span>[\s\S]*?<\/h1>/gi,
		"\n# $1\n"
	);
	cleaned = cleaned.replace(
		/<h2\b[^>]*>[\s\S]*?<span class="mw-headline"[^>]*>([\s\S]*?)<\/span>[\s\S]*?<\/h2>/gi,
		"\n## $1\n"
	);
	cleaned = cleaned.replace(
		/<h3\b[^>]*>[\s\S]*?<span class="mw-headline"[^>]*>([\s\S]*?)<\/span>[\s\S]*?<\/h3>/gi,
		"\n### $1\n"
	);
	cleaned = cleaned.replace(
		/<h4\b[^>]*>[\s\S]*?<span class="mw-headline"[^>]*>([\s\S]*?)<\/span>[\s\S]*?<\/h4>/gi,
		"\n#### $1\n"
	);
		...
		const promises = chunk.map(async (seed) => {
			...
			if (md) {
				...
				corpusDocs.push({
					...
					url:
						seed.lang === "fr"
							? `https://dragonball.fandom.com/fr/wiki/${seed.page}`
							: `https://dragonball.fandom.com/wiki/${seed.page}`,
					chars: md.length,
					markdown: md,
				});
			} else {

...
if (existsSync(ALIAS_MAP_PATH)) {
  ...
  console.warn("⚠ alias-map.json introuvable. Lancez `bun apps/bot/scripts/generate-alias-map.ts` d'abord.");
}
...
function detectLang(docId: string, docName: string, docUrl: string, content: string): string {
  ...
  if (idLower.includes("fd-fr-") || idLower.includes("-fr") || urlLower.includes("/fr/") || nameLower.includes("(fr)")) {
    return "fr";
  }
  if (idLower.includes("fd-en-") || idLower.includes("-en") || urlLower.includes("/en/") || nameLower.includes("(en)")) {
    return "en";
  }

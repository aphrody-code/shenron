...
describe("@rpbey/di", () => {
	...
	describe("util engine instances are singletons", () => {
		test("each built-in engine export is a distinct, non-null object", () => {
			...
			expect(defaultDependencyRegistryEngine).not.toBe(tsyringeDependencyRegistryEngine);
			expect(defaultDependencyRegistryEngine).not.toBe(typeDiDependencyRegistryEngine);
			expect(tsyringeDependencyRegistryEngine).not.toBe(typeDiDependencyRegistryEngine);
		});
	});
	...
	describe("DefaultDependencyRegistryEngine — real DI behaviour", () => {
		...
		test("clearAllServices empties the registry", () => {
			...
			expect(defaultDependencyRegistryEngine.getService(Service)).toBeInstanceOf(Service);

			defaultDependencyRegistryEngine.clearAllServices();
			expect(defaultDependencyRegistryEngine.getService(Service)).toBeUndefined();

...
export class TypeDiDependencyRegistryEngine extends AbstractConfigurableDependencyInjector<
	typeof Container
> {
	public static token = new Token<unknown>("@rpbey/discordy");
	...
	public static get instance(): TypeDiDependencyRegistryEngine {
		TypeDiDependencyRegistryEngine._instance ??= new TypeDiDependencyRegistryEngine();
		...
		if (this.useToken) {
			return (
				(this.injector
					...
					.find(
						(clazz) =>
							((clazz as Record<string, unknown>).constructor as unknown as T) === classType

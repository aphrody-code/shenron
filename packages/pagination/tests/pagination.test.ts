...
describe("@rpbey/pagination", () => {
	describe("createPagination — page math", () => {
		...
		test("rejects a negative totalItems", () => {
			expect(() => createPagination({ totalItems: -1 })).toThrow(/Pagination creation failed/);
		});

		test("rejects a non-integer pageSize", () => {
			expect(() => createPagination({ pageSize: 2.5, totalItems: 10 })).toThrow(
				/Page size must be a positive integer/
			);
		});

		test("rejects a zero pageSize", () => {
			expect(() => createPagination({ pageSize: 0, totalItems: 10 })).toThrow(
				/Page size must be a positive integer/
			);
		});

		test("rejects a maxPages below 1", () => {
			expect(() => createPagination({ maxPages: 0, totalItems: 10 })).toThrow(
	...
	describe("PaginationResolver — pure container", () => {
		...
		test("the stored sync resolver is callable and returns its item", () => {
			const r = new PaginationResolver((page) => ({ content: `p${page.toString()}` }), 3);

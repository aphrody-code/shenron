...
export class Example {
	...
	addExtend(command: SimpleCommandMessage, _client: Client, data: Data): unknown {
	...
	@SimpleCommand()
	findSource(command: SimpleCommandMessage, _client: Client, data: Data): unknown {
...
describe("Commands", () => {
	...
	it("Should execute simple command with two spaces", async () => {
		...
		const parsedCommand = await client.parseCommand(sampleMessage);
		const response = await client.executeCommand(sampleMessage);
		expect(response).toEqual(["!add plus second", ["car"], parsedCommand, true]);
	});
	...
	it("Should avoid splitter space for options", async () => {
		...
		for (const result of results) {
			expect(result.response).toEqual(["!add", ["2", "4"], result.parsedCommand, true]);
		}
	});
	...
	it("Should execute arg splitter regex", async () => {
		...
		const parsedCommand = await client.parseCommand(sampleMessage);
		const response = await client.executeCommand(sampleMessage);
		expect(response).toEqual(["!ban", [123, 99, "ban reason test", "cars"], parsedCommand, true]);
	});
});

...
export class InteractionHandler {
	...
	executeContextMenu(interaction: ContextMenuCommandInteraction): Awaited<unknown> {
		const applicationCommand = interaction.isUserContextMenuCommand()
			? this.client.applicationCommandUsers.find((cmd) => cmd.name === interaction.commandName)
			: this.client.applicationCommandMessages.find((cmd) => cmd.name === interaction.commandName);
	...
	private getApplicationCommandGroupTree(
		...
		const getOptionsTree = (option: Partial<CommandInteractionOption> | undefined): void => {
			if (!option) {
				return;
			}

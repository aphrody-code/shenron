...
export class ApplicationCommandManager {
	...
	private async getCommandsByGuild(): Promise<Map<string, DApplicationCommand[]>> {
	...
	private async initGuildApplicationCommands(
		...
		const { commandsToAdd, commandsToUpdate, commandsToSkip, commandsToDelete } =
			await this.categorizeGuildCommands(commands, discordCommands, botResolvedGuilds);
	...
	private async initGlobalApplicationCommands(retainDeleted: boolean): Promise<void> {
		...
		const discordCommands = allDiscordCommands.filter(
			(cmd) => !cmd.guild && cmd.type !== ApplicationCommandType.PrimaryEntryPoint
		);
		...
		if (bulkUpdate.length > 0) {
			await this.client.application.commands.set(bulkUpdate as ApplicationCommandData[]);
		}
	}
	...
	private logCommandChanges(
		...
		const { commandsToAdd, commandsToUpdate, commandsToSkip, commandsToDelete, retainDeleted } =
			changes;

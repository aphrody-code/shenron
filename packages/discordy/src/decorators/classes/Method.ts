...
import { type Awaitable, type DDiscord, DGuard, type GuardFunction } from "../../index.js";
...
export abstract class Method extends Decorator {
	 ...
	 */
	get execute() {
		return (guards: GuardFunction[], ...params: unknown[]): Promise<unknown> => {
	 ...
	 * @returns Function that executes the complete guard chain
	 */
	private createGuardChain(globalGuards: DGuard[]): (...params: unknown[]) => Promise<unknown> {

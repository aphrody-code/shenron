...
const conf = { base: "https://eu.api.ovh.com/1.0", ak: g("application_key")!, as: g("application_secret")!, ck: g("consumer_key")! };

async function call(method: string, path: string, body?: unknown) {
  const url = `${conf.base}${path}`;
  const bodyStr = body === undefined ? "" : JSON.stringify(body);
  const ts = Number(await (await fetch(`${conf.base}/auth/time`)).text());
  const sig = "$1$" + createHash("sha1").update(`${conf.as}+${conf.ck}+${method}+${url}+${bodyStr}+${ts}`).digest("hex");
  const headers: Record<string, string> = { "X-Ovh-Application": conf.ak, "X-Ovh-Consumer": conf.ck, "X-Ovh-Timestamp": String(ts), "X-Ovh-Signature": sig };
...
for (const [ft, ip] of [["A", IP4], ["AAAA", IP6]] as const) {
  const ids = (await call("GET", `/domain/zone/${ZONE}/record?fieldType=${ft}`)) as number[];
  for (const id of ids) {
    const rec = (await call("GET", `/domain/zone/${ZONE}/record/${id}`)) as { subDomain: string; target: string };
    if (!SUBS.has(rec.subDomain)) continue;
    const label = rec.subDomain || "@";
    if (rec.target === ip) { console.log(`= ${label} ${ft} déjà ${ip}`); continue; }
    console.log(`${DRY ? "[dry] " : ""}→ ${label} ${ft} ${rec.target} => ${ip}`);
    if (!DRY) await call("PUT", `/domain/zone/${ZONE}/record/${id}`, { target: ip });
  }
}
if (!DRY) { await call("POST", `/domain/zone/${ZONE}/refresh`); console.log("zone rafraîchie"); }
